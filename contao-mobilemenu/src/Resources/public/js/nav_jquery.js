(function ($) {
  ...
  $.XMobileMenu.prototype = {
    ...
    setModeSettings: function () {
      this.containerelement.css({ width: '100%', overflow: 'auto', 'z-index': 50000, margin: 0, left: 0, top: 0, position: 'absolute' });
    },
    setPosition: function () {

      var maxHeight = $(window).height() - (this.buttonelement.position().top + this.buttonelement.height());

      if (this.completeSize === true) {
        this.containerelement.css({ height: $(window).height() });
      } else {
        this.containerelement.css({ top: (this.buttonelement.position().top + this.buttonelement.height()), 'max-height': maxHeight + 'px', width: '100%' });
      }
